<template>
  <div class="form-container">
    ...
    <VForm ref="vformRef" @submit="submitAddStaff" :validation-schema="staffFormSchema" class="vform">
      <!-- Email -->
      ...
      <div class="form-group">
        ...
        <Field name="password" type="password" class="form-control" v-model="staff.password" />
...
const staffFormSchema = yup.object().shape({
  ...
  position: yup.string().oneOf(['librarian'], 'Vị trí không hợp lệ.'),
});
...
const submitAddStaff = async () => {
  ...
  await executeWithSwal(async () => {
    ...
    return await authService.signupForStaff(staff.value);
  }, false, true);
